...
Here is a breakdown of all the included workflows and how to use them:

### 1. Basic CI & Testing (`.github/workflows/ci.yml`)
- **What it does:** Automatically installs Python and runs unit tests (`test_math_operations.py`) every time code is pushed.
...
- **What it does:** Demonstrates the `workflow_dispatch` trigger, allowing manual execution of a workflow with custom text inputs.
...
- **How to test it:** Run the workflow manually and watch the visual graph. The `independent_check` and `build_job` run at the same time, while `deploy_job` waits for multiple jobs to finish.

### 4. Artifacts (`.github/workflows/artifacts.yml`)
- **What it does:** Demonstrates passing a text file from Job 1 to Job 2 across fresh virtual machines using `upload-artifact` and `download-artifact`.
...
- **How to test it:** Add messy code (like an unused import) to `math_operations.py` and run the workflow to see it fail.
...
- **How to access it:** Go to the main page of this repository, look at the **Packages** section on the right sidebar, and find the compiled Docker image ready to be pulled.
...
- **How to access it:** Visit the Public IP address of the EC2 instance in a web browser.
